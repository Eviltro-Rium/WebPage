...
(function (global) {
    ...
    proto._flashAccessorySlot = function (itemName) {
        ...
        if (!slots.length) return wait(280);
        slots.forEach(el => {
            el.classList.remove('acc-trigger-flash');
            void el.offsetWidth;
            el.classList.add('acc-trigger-flash');
        });
        return wait(480).then(() => {
            slots.forEach(el => el.classList.remove('acc-trigger-flash'));
        });
    };
})(window);
