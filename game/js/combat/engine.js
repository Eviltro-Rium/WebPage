...
(function () {
  ...
  class Engine{
    ...
    character(n,ai=false){let m=CharacterRegistry.get(n);return Object.assign({name:(ai?'AI ': '')+n,hp:m.hp,maxHp:m.hp,burn:0,frozen:false,bleed:0,poison:0,blind:0,iceSeal:0,guard:0,fly:0,parasite:0,alive:true,bloodthirst:false,bomb:0},m.init())}
    ...
    _canOfferOttoCrit(d,unblock){
      return this.name(this.s.player)==='Otto'
        && (Number(d)||0)>4
        && !unblock
    ...
    _hasPurifyableBuff(ch){
      if(!ch)return false;
      return !!(ch.burn||ch.bleed||ch.frozen||ch.poison||ch.blind||ch.iceSeal||ch.bomb||ch.guard||ch.fly||ch.crit||ch.lush||ch.parasite)
    }
    ...
    clearDebuffs(x){const S=this._status();if(S&&S.clearDebuffs){S.clearDebuffs(x);return}x.burn=0;x.bleed=0;x.poison=0;x.frozen=false;x.bomb=0;x.blind=0;x.iceSeal=0}
  ...
  Engine.prototype._acknowledgeEvents1v2=function(through){
    ...
    this.dealAttackHit(this.s[bombOwner]||this.s.ai,this.s.player,p.damage,isDrainAi);
