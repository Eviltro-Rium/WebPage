...
(function (global) {
    ...
    const EngineStatus = {
        ...
        burn(engine, entity, amount, opts) {
            ...
            entity.burn = Math.min(5, entity.burn + amount);
            if (!opts || opts.silent !== true) {
                const target = targetKey(engine, entity);
        ...
        bleed(engine, entity, amount, opts) {
            if (amount <= 0) return;
            entity.bleed = Math.min(3, entity.bleed + amount);
            if (!opts || opts.silent !== true) {
                const target = targetKey(engine, entity);
        ...
        addGuard(engine, entity, amount, opts) {
            ...
            entity.guard = Math.min(5, before + amount);
            const added = entity.guard - before;
        ...
        clean(entity, all = false, kind = null) {
            ...
            if (all) {
                ...
                entity.bomb = 0; entity.blind = 0; entity.iceSeal = 0; entity.guard = 0; entity.fly = 0;
                entity.lush = 0; entity.crit = 0; entity.parasite = 0; entity.chaos_red = false;
