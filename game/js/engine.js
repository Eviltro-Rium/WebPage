...
(function () {
  ...
  const ENGINE_EVENT_TYPES=(window.FurryGame&&window.FurryGame.CombatEvents&&window.FurryGame.CombatEvents.Types)||{HIT:'hit'};
  ...
  class Engine{
    ...
    hurt(x,n,kind=false,opts={}){n=Math.max(0,Number(n)||0);x.hp=Math.max(0,x.hp-n);x.alive=x.hp>0;if(this.name(x)==='Serenity')x.bloodthirst=x.hp<30;if(n>0&&!opts.silent){let w=x===this.s.player?'player':(x===this.s.ai2?'ai2':'ai');let isBleed=kind==='bleed'||kind===true,isPoison=kind==='poison',isDrain=kind==='drain';if(isBleed||isPoison||isDrain){let tag=isDrain?'吸血':isBleed?'流血':'中毒';this.emit('hurt',`-${n}[${tag}]`,null,{who:w,amount:n,bleed:isBleed,drain:isDrain,poison:isPoison})}else{this.emit(ENGINE_EVENT_TYPES.HIT,'受到'+n+'点伤害',null,{who:w,amount:n,kind:'damage'})}}}
