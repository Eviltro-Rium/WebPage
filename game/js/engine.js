...
(function () {
  ...
  class Engine{
    ...
    aiDefendLegal(c,top,x){if(c.isBlack||c.drawThree||c.drawTwo||c.potion||c.swapHand||c.shuffleToDeck)return true;if(c.superPurify)return x.debuff||x.oppGuard>0;if(c.purify)return x.debuff;if(c.isWhite)return c.value<=3&&this.legal(c,true);if(c.value<=3)return this.legal(c,true);if(this.s.isAdventure&&c.value>=4&&c.value<=6){let mod=this._getAdventureMod(this.name(this.s.ai));if(mod&&mod.canDefendHigh)return this.legal(c,true)}return false}
    ...
    _monsterHasDefendSkill(mod,card){if(!mod||!card)return false;if(card.isItemCard)return true;if(typeof mod.defendImmune==='function'&&mod.defendImmune(card))return true;if(typeof mod.defendHeal==='function'&&mod.defendHeal(card)>0)return true;if(typeof mod.defendPoison==='function'&&mod.defendPoison(card)>0)return true;if(typeof mod.defendBleed==='function'&&mod.defendBleed(card)>0)return true;if(typeof mod.defendCounter==='function'&&mod.defendCounter(card)>0)return true;if(typeof mod.defendBlock==='function'&&mod.defendBlock(card,8)>0)return true;if(typeof mod.defendDrawSelf==='function'&&mod.defendDrawSelf(card)>0)return true;if(typeof mod.defendSplit==='function'&&mod.defendSplit(card))return true;if(typeof mod.defendLush==='function'&&mod.defendLush(card)>0)return true;if(typeof mod.defendGuard==='function'&&mod.defendGuard(card)>0)return true;if(typeof mod.defendAllLush==='function'&&mod.defendAllLush(card)>0)return true;if(typeof mod.defendAllHeal==='function'&&mod.defendAllHeal(card)>0)return true;if(card.value===1||card.value===3)return true;return false}
    _chooseAIDefendAdventure(hand,top,incomingDamage=0){let x=this.aiContext({incomingDamage}),name=this.name(this.s.ai),mod=this._getAdventureMod(name);if(!mod)return null;for(const c of hand){if(c.isItemCard&&c.magic&&this.aiDefendLegal(c,top,x))return c}let numbers=hand.filter(c=>c.isNumberCard&&this.aiDefendLegal(c,top,x)).sort((a,b)=>b.value-a.value);for(const c of numbers){if(this._monsterHasDefendSkill(mod,c))return c}if(numbers.length)return numbers[0];for(const c of hand){if(this.aiDefendLegal(c,top,x))return c}return null}
