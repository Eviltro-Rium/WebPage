...
Serenity 生命低于 30 时获得[嗜血]印记。印记一旦获得便保持，不会因为治疗回到 30 以上而退出，也不能被净化或超级净化清除。只有尚未获得印记的正常状态会触发恢复额外+1生命。

- 代码：`engine.js` / `status.js` 的伤害、恢复与回合开始逻辑
...
冒险挑战房在第一名敌人出局后战斗继续，玩家仍按回合补牌；普通房、Boss 房以及挑战房最终胜利均不因击败敌人额外自动补牌。
...
## 15. Leon 0牌 — 随机弃对手2张手牌

Leon 0牌对所有存活对手 +2层灼烧、7点不可防御伤害，自身受到 `目标数×2` 点伤害，并由系统从所有存活对手手牌中随机弃掉至多2张。若对手手牌不足则弃掉所有可用手牌。

- 代码：`engine_1v2.js:265` `leonZero1v2()`

---

## 16. 抽取对手手牌 — 按模式选择策略

Chan 4/7、Saiki 5、Blaze 4、Moze 5、Leon 7 等会检查攻击目标手牌的技能，统一通过 `OpponentHandPolicy` 选择策略：冒险模式（包括测试房）因 NPC 手牌明牌，由玩家点击目标手牌后确认；经典 1v1/1v2 及在线模式继续使用共享随机源自动抽取。1v2 始终只从当前攻击目标的手牌中处理，不能选择另一名 NPC 的牌。抽到后仍按角色原规则进入判定区、玩家手牌或对应弃牌库。

- 代码：`js/combat/opponent_hand_policy.js`、`engine.js` `resolveOpponentHandSkill()` / `chooseOpponentCard()`；`engine_1v2.js` `play1v2()`

---

## 17. 对手无手牌时的自动处理

对手无手牌时按角色自动判定：Chan 4牌=2点伤害跳防御、Chan 7牌=6点、Saiki 5牌=4点、Blaze 4牌=2+灼烧加成、Moze 5牌=恢复2点+1层守护跳防御。
