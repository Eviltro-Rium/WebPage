...
Leon 0牌对所有存活对手 +1层灼烧、7点不可防御伤害，自身受到 `目标数×2` 点伤害，并由系统从所有存活对手手牌中随机弃掉至多2张。若对手手牌不足则弃掉所有可用手牌。
...
Chan 4/7、Saiki 3/5、Blaze 4、Moze 5、Leon 7 等会检查攻击目标手牌的技能，统一通过 `OpponentHandPolicy` 选择策略：冒险模式（包括测试房）因 NPC 手牌明牌，由玩家点击目标手牌后确认；经典 1v1/1v2 及在线模式继续使用共享随机源自动抽取。1v2 始终只从当前攻击目标的手牌中处理，不能选择另一名 NPC 的牌。抽到后仍按角色原规则进入判定区、玩家手牌或对应弃牌库。
