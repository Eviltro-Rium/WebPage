 ...
 *
 * 防御限制：只能打出 1~3 数字牌或魔法牌（4~6 不可防御）。
 * 防御优先级：魔法牌 → 1~3 数字牌降序。
 */
(function () {
  const NpcStrategy = {
    chooseAttack(hand) {
      for (let i = 0; i < hand.length; i++) if (hand[i].magic) return i;
    ...
    isDefendLegal(card) {
      ...
      return card.magic || (card.isNumberCard && card.value >= 1 && card.value <= 3);
    },
