/**
 * Adventure battle controller
 *
 * Owns the browser lifecycle around AdventureBattleEngine: session recovery,
 * persistence, UI mounting and settlement hand-off.  It deliberately contains
 * no combat rules; all card, turn and damage decisions live in the canonical
...
(function () {
  ...
  function saveAdventureProgress(engine) {
    ...
    if (typeof save.isSafePhase === 'function' && !save.isSafePhase(engine.s.phase)) return false;
    save.save(engine);
    return true;
  }
  ...
  function loadCombatSession() {
    const session = window.AdventureBattleSession;
    return session && typeof session.load === 'function' ? session.load() : null;
  }
  ...
  function buildSidebarHtml(snap) {
    ...
    if (accessories.length) {
      ...
      accessories.forEach(it => {
        ...
        if (!grouped[it.name]) { grouped[it.name] = { item: it, count: 0 }; order.push(it.name); }
      ...
      order.forEach(name => {
        ...
        const it = Object.assign({}, g.item, {
          displayName: g.item.displayName + (g.count > 1 ? ' ×' + g.count : '')
        });
        html += sidebarItemHtml(it);
      });
      html += '</div>';
    }
  ...
  // Rebuild the settlement overlay after a full page reload.  The adventure
  // engine is restored from localStorage by AdventureUI; unlike a combat
  // session there is no live battle engine to resume here.
  function resumeSettlement(eng, callback) {
    ...
    saveAdventureProgress(eng);
    showAdventureSettlement(eng, null, null, true);
    return true;
  }
  ...
  function ensureGameUI() {
    ...
    gameUI._showGameOver = function () {
      ...
      if (battleEngine.testMode) {
        schedule(() => leaveTestBattle(finalState, persistentState, playerWon), 420);
        return;
      }
    ...
    try {
      result = initialState.resumeBattle
        ...
        : battleEngine.startAdventure({
          ...
          opponentStage: initialState.opponentStage || null,
    ...
    try {
      result = initialState.resumeBattle
        ...
        : battleEngine.startAdventure1v2({
          ...
          stage: initialState.stage || 1,
          opponent1Stage: initialState.opponent1Stage || null,
          opponent2Stage: initialState.opponent2Stage || null,
          scene: initialState.scene || null,
          testMode: !!initialState.testMode,
          playerState: initialState.playerState || initialState,
          playerPile: initialState.playerPile || null,
