...
function makeSoilTexture() {
  ...
  ctx.lineWidth = 1.2;
  for (let i = 0; i < 18; i++) {
    const y = (i / 18) * size + Math.random() * 8;
    ctx.beginPath();
    ctx.moveTo(0, y);
...
export function initSceneBackground(containerId = 'sceneBackground') {
  ...
  for (let i = 0; i < terrainPos.count; i++) {
    ...
    tmpColor.copy(fieldLow).lerp(fieldHigh, heightT);
    tmpColor.lerp(soil, 0.28 + (1 - heightT) * 0.22 + (1 - furrow) * 0.12);
    // Subtle grit baked into vertex color so distant terrain still reads textured.
    const grit = 0.94 + ((Math.sin(x * 7.1 + z * 5.3) * 0.5 + 0.5) * 0.12);
